...
function delete_move_next_read($currloc)
{
    ...
    if (!(($where && $what) || ($currentArrayIndex == -1)) && !$passed_ent_id) {
        ...
        * echo "<base href=\"$location/\">" . */
        echo '<div class="delete-move-next" style="background:' . $color[9] . '">';

        if (((float)$prev) > 0){
            echo '<div class="move-next-action">';
            echo "<a href=\"read_body.php?passed_id=$prev_if_del&amp;mailbox=$urlMailbox&amp;sort=$sort&amp;startMessage=$startMessage&amp;show_more=0&amp;delete_id=$passed_id&amp;smtoken=" . sm_generate_security_token() . "\">" . _("Delete &amp; Prev") . "</a>" . "</div>";
            if ($delete_move_next_show_unread == 'on') {
                echo '<div class="move-next-action">';
                echo "<a href=\"read_body.php?passed_id=$prev_if_del&amp;mailbox=$urlMailbox&amp;sort=$sort&amp;startMessage=$startMessage&amp;show_more=0&amp;unread_id=$passed_id&amp;smtoken=" . sm_generate_security_token() . "\">" . _("Unread &amp; Prev") . "</a>" . "</div>";
            }
        }
        else {
            echo '<div class="move-next-action">';
            echo _("Delete &amp; Prev") . "</div>";
            if ($delete_move_next_show_unread == 'on') {
                echo '<div class="move-next-action">';
                echo _("Unread &amp; Prev") . "</div>";
            }
        }
        if (((float)$next) > 0){
            if ($delete_move_next_show_unread == 'on') {
                echo '<div class="move-next-action">';
                echo "<a href=\"read_body.php?passed_id=$next_if_del&amp;mailbox=$urlMailbox&amp;sort=$sort&amp;startMessage=$startMessage&amp;show_more=0&amp;unread_id=$passed_id&amp;smtoken=" . sm_generate_security_token() . "\">" . _("Unread &amp; Next") . "</a></div>";
            }
            echo '<div class="move-next-action">';
            echo "<a href=\"read_body.php?passed_id=$next_if_del&amp;mailbox=$urlMailbox&amp;sort=$sort&amp;startMessage=$startMessage&amp;show_more=0&amp;delete_id=$passed_id&amp;smtoken=" . sm_generate_security_token() . "\">" . _("Delete &amp; Next") . "</a></div>";
        } else {
            if ($delete_move_next_show_unread == 'on') {
                echo '<div class="move-next-action">';
                echo _("Unread &amp; Next") . "</div>";
            }
            echo '<div class="move-next-action">';
            echo _("Delete &amp; Next");
            echo '</div>';
...
function delete_move_next_moveNextForm($next)
{
    ...
    echo '<div class="message-mover" style="background:' . $color[9]. ';">'.
         "<form action=\"read_body.php?mailbox=$urlMailbox&amp;sort=$sort&amp;startMessage=$startMessage&amp;passed_id=$next\" method=\"post\">".
...
function delete_move_next_display_inside() {
    ...
    echo html_tag('h2', _("Delete/Unread/Move/Next Buttons:") ) . "\n".
         '<input type="checkbox" name="delete_move_next_ti" id="delete_move_next_ti"';
